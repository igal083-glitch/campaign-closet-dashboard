...
const STORAGE_KEY = "campaign-trading-journal-v1-4-clean-validation";
...
const starterData = [
  {
    ...
    chartImageName: "",
    journal: [
      { date: "02/05/2026", action: "כניסה", qty: "740", price: "1.58", note: "כניסת ליבה" },
    ],
  },
  {
    id: "NVAX-1",
    ...
    chartImageName: "",
    journal: [
      { date: "02/05/2026", action: "כניסה", qty: "980", price: "7.90", note: "כניסה ראשונה" },
    ],
  },
];
...
function normalizeRow(row = {}, options = {}) {
  const clearLegacyStop = Boolean(options.clearLegacyStop);
  return {
    id: row.id || makeId(row.ticker),
    date: row.date || today(),
    ticker: row.ticker || "",
    shares: row.shares || "",
    entry: row.entry || "",
    stop: clearLegacyStop ? "" : (row.stop || ""),
    lastAdd: row.lastAdd || "",
    ...
    pattern: row.pattern || "",
    thesis: row.thesis || "",
    addTrigger: row.addTrigger || "",
    plannedAddSize: row.plannedAddSize || "",
    localStop: row.localStop || "",
    addInvalidation: row.addInvalidation || "",
    chartImage: row.chartImage || "",
    chartImageName: row.chartImageName || "",
    journal: Array.isArray(row.journal) ? row.journal.map(normalizeJournalLine) : [],
  };
}
...
function isBuyAction(action) {
  return action === "כניסה" || action === "הוספה";
}

function isSellAction(action) {
  return action === "יציאה חלקית" || action === "יציאה מלאה";
}

function lineError(line) {
  if (line.action === "הערה") return "";
  if ((isBuyAction(line.action) || isSellAction(line.action)) && !validPrice(line.price)) return "חסר מחיר תקין";
  if ((isBuyAction(line.action) || line.action === "יציאה חלקית") && !validQty(line.qty)) return "חסרה כמות מניות";
  return "";
}
...
function getPositionMath(row) {
  ...
  const validBuyLines = journal.filter((j) => isBuyAction(j.action) && validQty(j.qty) && validPrice(j.price));

  if (validBuyLines.length === 0) {
...
function currentPrice(row) {
  return num(row.lastAdd) || num(row.entry);
}

function positionValue(row) {
  const qty = openShares(row);
...
function positionRisk(row) {
  ...
  if (!qty || !price || stop === null) return null;

  const risk = (price - stop) * qty;
...
function getExitPrice(row) {
  ...
  const line = [...(row.journal || [])].reverse().find((j) => j.action === "יציאה מלאה" && validPrice(j.price));
...
function getAddCount(row) {
  return (row.journal || []).filter((j) => j.action === "הוספה" && validQty(j.qty) && validPrice(j.price)).length;
}

function getLastAddLine(row) {
  return [...(row.journal || [])].reverse().find((j) => j.action === "הוספה" && validPrice(j.price));
}
...
function getTradeManagement(row) {
  ...
  if (!entry || !price) {
    return {
      ...
      structure: "חסר Entry או Current Price.",
      risk: "אין סטופ גלובלי מוגדר. הסיכון לא יחושב.",
      management: "השלם נתונים במגירה.",
      journalQuality: "בדוק Qty/Price ביומן.",
    };
  }

  const distanceFromEntry = ((price - entry) / entry) * 100;
  const riskDistance = stop ? ((price - stop) / price) * 100 : null;
  ...
  const invalidLines = (row.journal || []).filter((j) => lineError(j)).length;
  ...
  } else if (riskDistance !== null && distanceFromEntry <= 12 && riskDistance <= 15 && addCount <= 3) {
  ...
  const risk =
    riskDistance === null
      ? "לא הוגדר Global Stop. בלי סטופ אין חישוב סיכון פתוח."
      : riskDistance > 20
  ...
  const management =
    invalidLines > 0
      ? `יש ${invalidLines} שורות יומן לא תקינות. לתקן Qty/Price לפני החלטה.`
      : pnl !== null && pnl < 0
  ...
  const journalQuality = [
    ...
    invalidLines ? `${invalidLines} שורות שגויות` : "יומן תקין",
    lastAdd ? `הוספה אחרונה: ${lastAdd.price}` : "אין הוספה אחרונה",
  ].join(" | ");

  return { status, color, title, addDecision, structure, risk, management, journalQuality };
}
...
function isLightTheme(theme) {
  return theme === "white" || theme === "beige";
}

function panelClass(theme) {
  if (isLightTheme(theme)) return "rounded-xl border border-black/10 bg-black/5 p-4";
  return "rounded-xl border border-zinc-800 bg-black p-4";
}

function inputClass(theme, extra = "") {
  const base = isLightTheme(theme)
    ? "rounded border border-black/20 bg-white px-2 py-1 text-black"
    : "rounded border border-zinc-800 bg-zinc-950 px-2 py-1 text-white";
  return `${base} ${extra}`;
}
...
function openTradingViewWeb(ticker) {
  ...
  if (!symbol) return alert("אין טיקר לפתיחה");
  window.open(`https://www.tradingview.com/chart/?symbol=${encodeURIComponent(symbol)}`, "_blank");
}

function InfoCard({ label, value, color = "text-white", light = false }) {
  return (
    <div className={`min-h-[92px] rounded-xl border p-3 text-right ${light ? "border-black/10 bg-black/5" : "border-zinc-800 bg-black"}`}>
      <div className={light ? "text-xs text-zinc-600" : "text-xs text-zinc-500"}>{label}</div>
      <div className={`mt-2 break-words text-xl font-extrabold leading-tight ${color}`} dir="ltr">
        {value}
      </div>
    </div>
  );
...
export default function ClosetDashboard() {
  const [rows, setRows] = useState(() => {
    try {
      const saved =
        ...
        localStorage.getItem("campaign-trading-journal-v1-complete") ||
      ...
      return parsed.map((r) => normalizeRow(r, { clearLegacyStop: false }));
    } catch {
      return starterData.map(normalizeRow);
    }
  });
  ...
  const lightDrawer = isLightTheme(drawerTheme);
  ...
  const stats = useMemo(() => {
    ...
    const exposure = active.reduce((sum, row) => sum + (positionValue(row) || 0), 0);
    const risk = active.reduce((sum, row) => sum + (positionRisk(row) || 0), 0);
    const highestRisk = active
      .map((r) => ({ ticker: r.ticker, risk: positionRisk(r) || 0 }))
    ...
    return {
      ...
      exposure,
      risk,
      riskPercent: exposure ? (risk / exposure) * 100 : null,
      highestRiskTicker: highestRisk?.risk ? highestRisk.ticker : "—",
      highestRiskValue: highestRisk?.risk || null,
    };
  }, [rows]);
  ...
  function patchRow(id, updater) {
    setRows((prev) => prev.map((row) => (row.id === id ? normalizeRow(updater(row)) : row)));
  }
  ...
  async function loadLivePrices() {
    ...
    try {
      ...
      setRows((prev) =>
        prev.map((row) => (prices[row.ticker] ? { ...row, lastAdd: cleanNumber(prices[row.ticker], 2) } : row))
      );
    } catch {
  ...
  function addJournalLine() {
    ...
    patchRow(selected.id, (old) => ({
      ...old,
      journal: [...(old.journal || []), { date: today(), action: "הוספה", qty: "", price: "", note: "" }],
    }));
  }
  ...
  function updateJournal(index, field, value) {
    ...
    patchRow(selected.id, (old) => {
      ...
      journal[index] = normalizeJournalLine({ ...journal[index], [field]: value });

      const exit = [...journal].reverse().find((j) => j.action === "יציאה מלאה" && validPrice(j.price));
  ...
  function cleanBadJournalData() {
    if (!selected) return;
    patchRow(selected.id, (old) => ({
      ...old,
      stop: "",
      journal: (old.journal || []).map((line) => ({
        ...line,
        qty: validQty(line.qty) ? cleanNumber(line.qty, 2) : "",
        price: validPrice(line.price) ? cleanNumber(line.price, 2) : "",
      })),
    }));
  }
  ...
  function exportBackup() {
    const name = window.prompt("איך לקרוא לקובץ?", "campaign-journal-backup") || "campaign-journal-backup";
  ...
  function importBackup(file) {
    ...
    reader.onload = () => {
      try {
        ...
        const cleaned = imported.map(normalizeRow);
        setRows(cleaned);
  ...
  const cards = [
    ...
    ["Win Rate", percent(stats.winRate), "text-blue-300"],
    ["רווח סגור", money(stats.totalPnl), stats.totalPnl >= 0 ? "text-emerald-300" : "text-red-400"],
    ["חשיפה פתוחה", money(stats.exposure), "text-blue-300"],
    ["סיכון פתוח", stats.risk ? money(stats.risk) : "—", stats.risk > 500 ? "text-red-400" : "text-amber-300"],
    ["סיכון גבוה", stats.highestRiskValue ? `${stats.highestRiskTicker} ${money(stats.highestRiskValue)}` : "אין נתונים", "text-red-400"],
  ];
    ...
    <div className="min-h-screen bg-black p-6 text-white" dir="rtl">
      <div className="mb-6 rounded-2xl border border-amber-500/40 bg-zinc-950 p-5">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="text-right">
            <div className="mb-1 inline-flex rounded-full border border-amber-500/40 bg-amber-500/10 px-3 py-1 text-xs font-bold text-amber-300">
              V1.4 CLEAN · TRADING JOURNAL · לא Watchlist
            </div>
            <h1 className="mt-2 text-4xl font-extrabold">Campaign Trading Journal</h1>
      ...
      <div className="mb-5 grid gap-3 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5">
        {cards.map(([label, value, color]) => (
          <div key={label} className="min-h-[96px] rounded-xl border border-zinc-800 bg-zinc-950 p-4 text-right">
            <div className="text-xs text-zinc-500">{label}</div>
            <div className={`mt-2 break-words text-2xl font-extrabold leading-tight ${color}`} dir="ltr">
              {value}
            </div>
          </div>
        ))}
      ...
      <div className="overflow-x-auto rounded-xl border border-zinc-800">
        <div className="grid min-w-[1380px] grid-cols-[100px_120px_110px_90px_110px_120px_120px_120px_90px_95px_100px_80px_90px] gap-3 bg-zinc-900 p-3 text-sm font-bold text-zinc-300">
          <div>תאריך</div>
          <div>טיקר</div>
          <div>כמות פתוחה</div>
          <div>Avg</div>
        ...
        {visibleRows.map((row, index) => (
          <div
            key={row.id}
            className="grid min-w-[1380px] grid-cols-[100px_120px_110px_90px_110px_120px_120px_120px_90px_95px_100px_80px_90px] items-center gap-3 border-t border-zinc-800 p-3"
          >
            <div className="rounded border border-zinc-800 bg-black px-2 py-1 text-center" dir="ltr">{row.date}</div>
            ...
            <input
              ...
              className={`rounded border px-2 py-1 text-center font-bold ${
                row.ticker ? "border-zinc-800 bg-zinc-950 text-zinc-300" : "border-amber-500 bg-black text-white"
              }`}
              dir="ltr"
            />

            <div className="rounded border border-zinc-800 bg-black px-2 py-1 text-center font-bold text-amber-300" dir="ltr">
              {openShares(row) ? cleanNumber(openShares(row), 2) : "—"}
            </div>

            <div className="rounded border border-zinc-800 bg-black px-2 py-1 text-center font-bold text-blue-300" dir="ltr">
              {avgCost(row) ? cleanNumber(avgCost(row), 2) : "—"}
            </div>

            <div className="rounded border border-zinc-800 bg-black px-2 py-1 text-center font-bold text-red-400" dir="ltr">
              {hasExplicitStop(row) ? cleanNumber(row.stop, 2) : "—"}
            </div>

            <div className="rounded border border-emerald-500/40 bg-emerald-500/10 px-2 py-1 text-center font-bold text-emerald-300" dir="ltr">
            ...
            <div className="rounded border border-zinc-800 bg-black px-2 py-1 text-center font-bold text-emerald-300" dir="ltr">
              {currentPrice(row) ? cleanNumber(currentPrice(row), 2) : "—"}
            </div>

            <select
              value={row.status}
              onChange={(e) => patchRow(row.id, (old) => ({ ...old, status: e.target.value }))}
            ...
            <div className="rounded border border-zinc-800 bg-black px-2 py-1 text-center text-amber-300" dir="ltr">
              {row.exitPrice || "—"}
            </div>
            ...
            <button onClick={() => openTradingViewWeb(row.ticker)} className="rounded border border-blue-500 px-3 py-1 text-xs font-bold text-blue-300">
              Web
            </button>
          </div>
        ))}
      ...
      <div className="mt-2 text-right text-xs text-zinc-500">
        שים לב: הטבלה הראשית היא תצוגה בלבד. עדכון Qty / Price / Stop מתבצע במגירה.
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-md rounded-xl border border-zinc-800 bg-zinc-950 p-5 shadow-2xl">
            ...
            <div className="space-y-3 text-right text-sm text-zinc-300">
              <div><span className="font-bold text-amber-300">Equity Curve:</span><br />גרף שמראה את הרווח או ההפסד המצטבר שלך לאורך זמן.</div>
              <div><span className="font-bold text-amber-300">Equity סגור:</span><br />סך הרווח או ההפסד בפועל מעסקאות שכבר נסגרו.</div>
              <div><span className="font-bold text-amber-300">Peak:</span><br />השיא הגבוה ביותר שהגעת אליו ברווח המצטבר.</div>
              <div><span className="font-bold text-amber-300">Max DD:</span><br />הירידה הכי גדולה מהשיא. זה Drawdown — מדד סיכון חשוב מאוד.</div>
            </div>
            <div className="mt-5 flex justify-end">
              <button onClick={() => setShowEquityHelp(false)} className="rounded bg-amber-500 px-4 py-2 text-sm font-bold text-black">סגור</button>
            </div>
          </div>
        </div>
      )}
      ...
      {selected && (
        <div className={`mt-6 rounded-xl border p-4 ${drawerThemeClass(drawerTheme)}`}>
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <button onClick={() => setSelectedId("")} className="rounded border border-zinc-700 px-3 py-2 text-sm">
              סגור מגירה
            </button>

            <div className="flex flex-wrap gap-2">
              {["black", "gray", "white", "beige"].map((theme) => (
                <button
                  key={theme}
                  onClick={() => setDrawerTheme(theme)}
                  className={`rounded px-3 py-1 text-xs font-bold ${
                    drawerTheme === theme ? "bg-amber-500 text-black" : "border border-zinc-700"
                  }`}
                >
                  {theme === "black" ? "שחור" : theme === "gray" ? "אפור" : theme === "white" ? "לבן" : "בז׳"}
                </button>
              ))}
            </div>

            <div className="text-right">
              <h2 className="text-2xl font-extrabold" dir="ltr">{selected.ticker || "NEW"}</h2>
              <div className={lightDrawer ? "text-sm text-zinc-600" : "text-sm text-zinc-500"}>
                Campaign Journal Drawer · {selected.status === "סגור" ? "היסטוריה" : "עסקה פעילה"}
              </div>
            </div>
          </div>
          ...
          {drawerTab === "overview" && (
            <div className="space-y-5">
              <div className="grid gap-3 md:grid-cols-6">
                <InfoCard light={lightDrawer} label="Entry" value={selected.entry || "—"} color="text-blue-300" />
                <InfoCard light={lightDrawer} label="Global Stop" value={hasExplicitStop(selected) ? cleanNumber(selected.stop, 2) : "—"} color="text-red-400" />
                <InfoCard light={lightDrawer} label="Current Price" value={currentPrice(selected) ? cleanNumber(currentPrice(selected), 2) : "—"} color="text-emerald-400" />
                <InfoCard light={lightDrawer} label="Open Shares" value={openShares(selected) ? cleanNumber(openShares(selected), 2) : "—"} color="text-amber-300" />
                <InfoCard light={lightDrawer} label="Avg" value={avgCost(selected) ? cleanNumber(avgCost(selected), 2) : "—"} color="text-blue-300" />
                <InfoCard light={lightDrawer} label="Position" value={money(positionValue(selected))} color="text-emerald-300" />
                <InfoCard light={lightDrawer} label="Live PnL" value={money(unrealizedPnl(selected))} color={(unrealizedPnl(selected) || 0) >= 0 ? "text-emerald-300" : "text-red-400"} />
                <InfoCard light={lightDrawer} label="Risk $" value={positionRisk(selected) ? money(positionRisk(selected)) : "—"} color="text-red-400" />
                <InfoCard light={lightDrawer} label="Risk %" value={percent(riskPercent(selected))} color="text-amber-300" />
                <InfoCard light={lightDrawer} label="Adds" value={getAddCount(selected)} color="text-amber-300" />
                <InfoCard light={lightDrawer} label="Pattern" value={selected.pattern || "—"} color="text-amber-300" />
                <InfoCard light={lightDrawer} label="Duration" value={durationDays(selected) ?? "—"} color="text-amber-300" />
              </div>

              <div className={panelClass(drawerTheme)}>
                <div className="mb-3 text-right text-sm font-bold text-amber-300">ניהול נתוני ליבה</div>
                <div className="mb-3 rounded border border-blue-500/30 bg-blue-500/10 p-3 text-right text-xs text-blue-300">
                  כאן מעדכנים נתוני ליבה בלבד. Qty ביומן = מספר מניות. Price = מחיר למניה. Global Stop הוא השדה היחיד שמחשב סיכון.
                </div>
                <div className="grid gap-3 md:grid-cols-4">
                  <div>
                    <div className="mb-1 text-xs text-zinc-500">Core Qty</div>
                    <input value={selected.shares || ""} onChange={(e) => updateSelected("shares", e.target.value)} className={`w-full p-2 text-center text-sm text-amber-300 ${inputClass(drawerTheme)}`} dir="ltr" />
                  </div>
                  <div>
                    <div className="mb-1 text-xs text-zinc-500">Entry</div>
                    <input value={selected.entry || ""} onChange={(e) => updateSelected("entry", e.target.value)} className={`w-full p-2 text-center text-sm text-blue-300 ${inputClass(drawerTheme)}`} dir="ltr" />
                  </div>
                  <div>
                    <div className="mb-1 text-xs text-zinc-500">Current Price</div>
                    <input value={selected.lastAdd || ""} onChange={(e) => updateSelected("lastAdd", e.target.value)} className={`w-full p-2 text-center text-sm text-emerald-300 ${inputClass(drawerTheme)}`} dir="ltr" />
                  </div>
                  <div>
                    <div className="mb-1 text-xs text-zinc-500">Global Stop</div>
                    <input value={selected.stop || ""} onChange={(e) => updateSelected("stop", e.target.value)} placeholder="ריק = אין סטופ" className={`w-full p-2 text-center text-sm text-red-400 ${inputClass(drawerTheme)}`} dir="ltr" />
                  </div>
                </div>

                <div className="mt-4 flex flex-wrap gap-2">
                  <button onClick={() => updateSelected("stop", "")} className="rounded border border-red-500 px-4 py-2 text-sm font-bold text-red-400">
                    נקה Global Stop
                  </button>
                  <button onClick={cleanBadJournalData} className="rounded border border-amber-500 px-4 py-2 text-sm font-bold text-amber-300">
                    נקה שדות שגויים
                  </button>
                  <button onClick={() => openTradingViewWeb(selected.ticker)} className="rounded border border-blue-500 px-4 py-2 text-sm font-bold text-blue-300">
                    פתח TradingView
                  </button>
                </div>
              </div>
          ...
          {drawerTab === "review" && (
            <div className="space-y-4">
              {tradeManagement && selected.status !== "סגור" ? (
                ...
                <div className={panelClass(drawerTheme)}>AI Review מוצג רק לעסקאות פעילות.</div>
              )}
            </div>
          )}
          ...
          {drawerTab === "addPlan" && (
            <div className={panelClass(drawerTheme)}>
              <div className="mb-3 text-right text-sm font-bold text-amber-300">Add Plan / תוכנית הוספה</div>
              <div className="grid gap-3 md:grid-cols-2">
                <div>
                  <div className="mb-1 text-xs text-zinc-500">Trigger / תנאי הוספה</div>
                  <input value={selected.addTrigger || ""} onChange={(e) => updateSelected("addTrigger", e.target.value)} placeholder="לדוגמה: Higher Low מעל 4.20" className={`w-full p-2 text-right text-sm ${inputClass(drawerTheme)}`} />
                </div>
                <div>
                  <div className="mb-1 text-xs text-zinc-500">Planned Add Size</div>
                  <input value={selected.plannedAddSize || ""} onChange={(e) => updateSelected("plannedAddSize", e.target.value)} placeholder="150" className={`w-full p-2 text-center text-sm text-amber-300 ${inputClass(drawerTheme)}`} dir="ltr" />
                </div>
                <div>
                  <div className="mb-1 text-xs text-zinc-500">Local Stop</div>
                  <input value={selected.localStop || ""} onChange={(e) => updateSelected("localStop", e.target.value)} placeholder="3.95" className={`w-full p-2 text-center text-sm text-red-400 ${inputClass(drawerTheme)}`} dir="ltr" />
                </div>
                <div>
                  <div className="mb-1 text-xs text-zinc-500">Invalidation / ביטול</div>
                  <input value={selected.addInvalidation || ""} onChange={(e) => updateSelected("addInvalidation", e.target.value)} placeholder="Break below mini base" className={`w-full p-2 text-right text-sm ${inputClass(drawerTheme)}`} />
                </div>
              </div>
            </div>
          )}
          ...
          {drawerTab === "journal" && (
            <div className={panelClass(drawerTheme)}>
              <div className="mb-3 rounded border border-amber-500/30 bg-amber-500/10 p-3 text-right text-xs text-amber-200">
                כלל חישוב: Qty = מספר מניות בלבד. Price = מחיר למניה. אין עמודת סטופ ביומן כדי למנוע בלבול — סטופ נמצא רק ב־Overview תחת Global Stop.
              </div>

              <div className="mb-3 flex items-center justify-between">
                <button onClick={addJournalLine} className="rounded bg-emerald-600 px-3 py-2 text-sm font-bold text-white">
                  פעולה +
                </button>
              ...
              <div className="overflow-x-auto">
                <div className="grid min-w-[900px] grid-cols-[120px_150px_120px_140px_1fr_80px] gap-2 border-b border-zinc-700 pb-2 text-xs font-bold text-zinc-500">
                  <div>תאריך</div>
                  <div>פעולה</div>
                  <div>Qty מניות</div>
                  <div>Price למניה</div>
                ...
                {(selected.journal || []).map((line, i) => {
                  ...
                  return (
                    <div key={i} className="grid min-w-[900px] grid-cols-[120px_150px_120px_140px_1fr_80px] items-center gap-2 border-b border-zinc-800/60 py-2">
                      <input value={line.date} onChange={(e) => updateJournal(i, "date", e.target.value)} className={inputClass(drawerTheme, "text-center")} dir="ltr" />

                      <select value={line.action} onChange={(e) => updateJournal(i, "action", e.target.value)} className={inputClass(drawerTheme)}>
                      ...
                      <input
                        ...
                        placeholder="62 / 75 / 980"
                        className={`${inputClass(drawerTheme, "text-center text-amber-300")} ${err.includes("כמות") ? "border-red-500" : ""}`}
                        dir="ltr"
                      />
                      ...
                      <input
                        ...
                        placeholder="1.58 / 4.20"
                        className={`${inputClass(drawerTheme, "text-center")} ${err.includes("מחיר") ? "border-red-500" : ""}`}
                        dir="ltr"
                      />

                      <input value={line.note || ""} onChange={(e) => updateJournal(i, "note", e.target.value)} placeholder={err || "הערה"} className={`${inputClass(drawerTheme)} ${err ? "border-red-500 placeholder:text-red-400" : ""}`} />

                      <button onClick={() => deleteJournal(i)} className="rounded bg-red-700 px-3 py-1 text-sm font-bold text-white">×</button>
                    </div>
                  );
                })}
              </div>
          ...
          {drawerTab === "chart" && (
            <div className="grid gap-4 md:grid-cols-2">
              <div className={panelClass(drawerTheme)}>
                <div className="mb-3 text-right text-sm font-bold text-amber-300">תבנית + תזה</div>

                <select value={selected.pattern || ""} onChange={(e) => updateSelected("pattern", e.target.value)} className={`mb-4 w-full p-2 text-right text-sm text-amber-300 ${inputClass(drawerTheme)}`}>
                ...
                <textarea value={selected.thesis || ""} onChange={(e) => updateSelected("thesis", e.target.value)} className={`h-28 w-full p-2 text-right text-sm ${inputClass(drawerTheme)}`} />

                <div className="mt-3 rounded border border-zinc-800 bg-black/10 p-3">
                  <div className="mb-2 text-right text-xs font-bold text-zinc-500">קובץ גרף / צילום מסך</div>
                  <input type="file" accept="image/*" onChange={(e) => uploadChart(e.target.files?.[0])} className="w-full rounded border border-zinc-700 bg-black p-2 text-sm text-zinc-300" />
              ...
              <div className={panelClass(drawerTheme)}>
                ...
                {selected.chartImage ? (
                  ...
                  <div className="rounded border border-zinc-800 bg-black/10 p-6 text-center text-sm text-zinc-500">
                    אין צילום גרף שמור לטיקר הזה.
                  </div>
                )}
              </div>
            </div>
      ...
      <div className="mt-8 space-y-6">
        <div className="grid gap-3 lg:grid-cols-2">
          ...
          <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-4">
            <div className="mb-3 flex items-center justify-between">
              <button onClick={() => setShowEquityHelp(true)} className="rounded border border-zinc-700 px-3 py-1 text-xs font-bold text-zinc-300 hover:border-amber-500 hover:text-amber-300">
                ? הסבר
              </button>
              <div className="text-right text-sm font-bold text-amber-300">Equity Curve + Drawdown</div>
            </div>
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-4">
          ...
          <div className="overflow-x-auto">
            ...
            {patternPerformance.map((p) => (
              <div key={p.pattern} className="grid min-w-[720px] grid-cols-[180px_80px_100px_120px_120px_120px] gap-2 border-b border-zinc-900 py-2 text-sm">
                <div>{p.pattern}</div>
                <div>{p.trades}</div>
            ...
            {patternPerformance.length === 0 && <div className="py-3 text-sm text-zinc-500">אין עדיין עסקאות סגורות עם נתוני תבנית.</div>}
          </div>
        </div>
